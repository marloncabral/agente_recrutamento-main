...
import utils # CORREÇÃO: Importa o nome correto do arquivo, 'utils'.
...
def criar_dataframe_mestre():
    ...
    Carrega os dados usando as funções padronizadas de utils.
    """
...
def preparar_dados_para_treino(df):
    ...
    if df_modelo.empty or df_modelo["target"].nunique() < 2:
        ...
        st.warning(f"Não foi possível treinar o modelo de ML. Feedbacks encontrados: {list(unique_feedbacks)}")
        return None
