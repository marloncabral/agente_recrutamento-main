...
# --- ETAPA 3: Funções de IA Generativa ---
# CORREÇÃO: As funções do Gemini foram adicionadas aqui, no escopo do app.
def analisar_competencias_vaga(competencias_texto, api_key):
    if not api_key: return None
    try:
        model = genai.GenerativeModel('gemini-1.5-flash')
        prompt = f"""Analise a descrição de competências de uma vaga de TI e extraia as informações em formato JSON. Descrição: "{competencias_texto}". Seu objetivo é identificar: 1. `obrigatorias`: Uma lista das 5 competências técnicas mais essenciais. 2. `desejaveis`: Uma lista de outras competências. 3. `sinonimos`: Para cada competência obrigatória, gere uma lista de 2-3 sinônimos ou tecnologias relacionadas. Retorne APENAS o objeto JSON."""
...
def gerar_relatorio_final(vaga, candidato, historico_chat, api_key):
    ...
    prompt = f"""Você é um especialista em recrutamento da Decision. Analise a transcrição de uma entrevista e gere um relatório final. **Vaga:** {vaga.get('titulo_vaga', 'N/A')}. **Candidato:** {candidato.get('nome', 'N/A')}. **Transcrição:**\n{historico_chat}. **Sua Tarefa:** Gere um relatório estruturado no formato markdown com Score Geral (0-10), Pontos Fortes, Pontos de Atenção e Recomendação Final."""
    try:
        model = genai.GenerativeModel('gemini-1.5-flash')
        response = model.generate_content(prompt)
...
def gerar_analise_comparativa(vaga, relatorios, api_key):
    ...
    prompt = f"""Você é um Diretor de Recrutamento da Decision. Sua tarefa é criar um parecer final para apresentar ao seu cliente, a empresa '{cliente}'. Analise os relatórios de entrevista dos finalistas para a vaga de {vaga.get('titulo_vaga', 'N/A')}. **Relatórios:**\n{relatorios}\n**Sua Tarefa:** Crie um ranking dos candidatos e escreva um parecer final justificando a recomendação do candidato ideal para o cliente."""
    try:
        model = genai.GenerativeModel('gemini-1.5-flash')
        response = model.generate_content(prompt)
        return response.text
...
with st.sidebar:
    ...
    if modelo_match:
        ...
        st.warning("⚠️ Fallback Ativado: O modelo de ML não pôde ser treinado. O sistema usará a IA Generativa para o matching.")
...
# --- Interface Principal ---
st.title("✨ Assistente de Recrutamento da Decision v2.3")
st.markdown("Bem-vindo ao assistente híbrido de IA.")
...
with tab1:
    if st.session_state.get('ml_mode_available', False):
        ...
        st.warning("Não foi possível treinar o modelo de ML. O sistema está usando a IA Generativa para analisar as competências.")
    ...
    if 'df_analise_resultado' in st.session_state and not st.session_state.df_analise_resultado.empty:
        ...
        df_editado = st.data_editor(
            ...
            column_config={"selecionar": st.column_config.CheckboxColumn("Selecionar", default=False), "score": score_column_config},
    ...
    if not google_api_key:
        ...
        if prompt := st.chat_input("Digite a resposta do candidato..."):
            ...
            with st.spinner("Agente 2 está pensando..."):
                ...
                prompt_ia = f"""Você é um entrevistador de IA da Decision. Sua tarefa é conduzir uma entrevista concisa e eficaz, fazendo UMA PERGUNTA DE CADA VEZ. **Regras:** 1. Você está na pergunta {question_count + 1} de um total de 5 a 7 perguntas. 2. Analise o histórico e o contexto para formular a próxima pergunta lógica. 3. NÃO FAÇA MAIS DE UMA PERGUNTA. 4. Se já fez 6 ou 7 perguntas, finalize a entrevista. **Contexto Vaga:** {vaga_atual.get('titulo_vaga')} | {vaga_atual.get('perfil_vaga_texto')}. **Contexto Candidato:** {candidato_atual.get('nome')} | {candidato_atual.get('candidato_texto_completo')}. **Histórico:** {historico_formatado}. **Sua próxima ação:**"""
                model = genai.GenerativeModel('gemini-1.5-flash')
